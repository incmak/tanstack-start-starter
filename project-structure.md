...
src
|
+-- routes/              # TanStack Router file-based routes (framework default structure)
|   +-- __root.tsx       # root layout component with HTML shell
|   +-- index.tsx        # home page route (/)
|
+-- router.tsx           # router configuration (creates router instance)
+-- routeTree.gen.ts     # auto-generated route tree (DO NOT EDIT - managed by TanStack Router plugin)
|
...
|
+-- features             # feature based modules (main code organization)
|
...
**TanStack Router/Start Specifics:**

- **File-based routing**: Routes are defined by creating files in `src/routes/`. The router plugin auto-generates `routeTree.gen.ts`.
- **Route conventions**:
  - `__root.tsx` - Root layout (wraps all routes)
  - `index.tsx` - Maps to `/`
  - `about.tsx` - Maps to `/about`
  - `posts.$id.tsx` - Maps to `/posts/:id` (dynamic route)
  - `posts_.$id.tsx` - Maps to `/posts/:id` (pathless layout route)
- **Router setup**: `src/router.tsx` exports a `getRouter()` function that creates the router instance
...
To forbid cross-feature imports, you can use Biome or ESLint. Here's an example with ESLint:
...
To enforce this, you can use Biome or ESLint. Here's an example with ESLint:
